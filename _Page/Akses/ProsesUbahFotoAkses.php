<?php
    ...
    //Buka File Lama
    $ImageLama=GetDetailData($Conn,'admin','id_admin',$id_access,'admin_image');
    ...
    if($tipe_gambar == "image/jpeg"||$tipe_gambar == "image/jpg"||$tipe_gambar == "image/gif"||$tipe_gambar == "image/png"){
        if($ukuran_gambar<2000000){
            if(move_uploaded_file($tmp_gambar, $path)){
                $UpdateAkses = mysqli_query($Conn,"UPDATE admin SET 
                    admin_image='$namabaru'
                WHERE id_admin='$id_access'") or die(mysqli_error($Conn)); 
                if($UpdateAkses){
                    if(!empty($ImageLama)){
